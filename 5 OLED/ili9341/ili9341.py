# MicroPython IL9341 LCD Display driver, SPI interface
# Original source code: https://github.com/rdagger/micropython-ili9341
...
# command definitions from ILI9341 datasheet
...
# Subclassing FrameBuffer DOES NOT WORK!
...
class ILI9341(object):
    def __init__(self, width, height, spi, dc, rst, cs):
        ...
        Args:
            width (int): Screen width (default 320)
            height (int): Screen height (default 240)
            spi (Class Spi):  SPI interface
...
#         super().__init__(self.buffer, self.width, self.height, framebuf.RGB565)
    ...
    def init_display(self):
        ...
        sleep(.1)
        self.write_cmd(PWCTRB, 0x00, 0xC1, 0x30)  # Pwr ctrl B
        self.write_cmd(POSC, 0x64, 0x03, 0x12, 0x81)  # Pwr on seq. ctrl
        self.write_cmd(DTCA, 0x85, 0x00, 0x78)  # Driver timing ctrl A
        self.write_cmd(PWCTRA, 0x39, 0x2C, 0x00, 0x34, 0x02)  # Pwr ctrl A
        self.write_cmd(PUMPRC, 0x20)  # Pump ratio control
        self.write_cmd(DTCB, 0x00, 0x00)  # Driver timing ctrl B
        self.write_cmd(PWCTR1, 0x23)  # Pwr ctrl 1
        self.write_cmd(PWCTR2, 0x10)  # Pwr ctrl 2
        self.write_cmd(VMCTR1, 0x3E, 0x28)  # VCOM ctrl 1
        self.write_cmd(VMCTR2, 0x86)  # VCOM ctrl 2
        self.write_cmd(MADCTL, self.rotation)  # Memory access ctrl
        self.write_cmd(VSCRSADD, 0x00)  # Vertical scrolling start address
        self.write_cmd(PIXFMT, 0x55)  # COLMOD: Pixel format
        self.write_cmd(FRMCTR1, 0x00, 0x18)  # Frame rate ctrl
        self.write_cmd(DFUNCTR, 0x08, 0x82, 0x27)
        self.write_cmd(ENABLE3G, 0x00)  # Enable 3 gamma ctrl
        self.write_cmd(GAMMASET, 0x01)  # Gamma curve selected
        self.write_cmd(GMCTRP1, 0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08, 0x4E,
                       0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00)
        self.write_cmd(GMCTRN1, 0x00, 0x0E, 0x14, 0x03, 0x11, 0x07, 0x31,
    ...
    def invert(self, invert):
        self.write_cmd(INVOFF | (invert & 1))

    def rotate(self, angle):
        if angle in [0, 90 , 180, 270]:
            self.rotation = ROTATE[angle]
            self.write_cmd(MADCTL, self.rotation)  # Memory access ctrl

    def show(self):
        self.block(0, 0, self.width, self.height, self.buffer)
    ...
    def block(self, x0, y0, x1, y1, data):
        ...
        """
        self.write_cmd(SET_COLUMN, *ustruct.pack(">HH", x0, x1))
        self.write_cmd(SET_PAGE, *ustruct.pack(">HH", y0, y1))
